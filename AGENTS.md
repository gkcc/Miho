...
- 本机交付默认直接构建并更新当前可运行的 Tauri 程序；NSIS、安装器、portable/发布包仅在用户明确要求时处理，也不为它们扩大测试范围。
...
- 运行时代码、配置、资源或依赖变化时重建并更新当前 Tauri 程序；只有用户明确要求安装器/发布包时才进入旧发布链。纯流程、说明或测试文档提交不冒充新的程序版本。
