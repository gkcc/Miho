...
该草案只读取用户显式传入的官方 ZZZ box 总览图，输出脱敏 roster JSON / Markdown，不保存 header UID、昵称或原始识别文本块，不读取 cookie/token，不写正式数据库。`needs_review_count > 0` 时只能作为候选 box 输入，人工确认前不能当作最终已拥有 box。

box 图识别结果如果存在待复核项，价值报告仍可生成，但必须在报告中标明“候选 box”，避免把识别草案误当成确定 box。
...
2. 继续收口 box roster 识别草案：
   ...
   - 识别草案只作为候选 box；
   - 人工确认前不得当作最终 box，后续如果要自动汇入必须另开单独任务设计确认流程。
