...
当前不再让 Codex 自动操作右侧 ChatGPT 页面。已经验证过：右侧输入框可写入，但发送、截图和多轮定位容易被页面长请求拖住，继续尝试只会浪费上下文。

固定做法：

1. 运行 `dist\MihoProbe.exe ask-gpt --focus "..." --copy`。
2. 如果复制成功，用户直接粘贴到右侧 GPT 并点击发送。
3. 如果复制失败，打开 `data/probes/demo/gpt_review_prompt.md`，复制全文发给右侧 GPT。
4. 用户把右侧 GPT 的结构化回复贴回当前 Codex 线程。
