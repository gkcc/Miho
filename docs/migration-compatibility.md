...
- Five report commands share the `miho-app` native executor, so CLI no longer owns a second path/clock/render/transaction implementation. `TaskIntentV1` is strict and pathless; pathful native request types are not serializable/deserializable. The process-local TaskManager and Tauri capabilities/workspace/start/get/list/cancel backend are migrated with cooperative pre-commit cancellation, opaque workspace IDs, public redaction and contiguous recoverable task updates. The safe frontend task UI and workspace-scoped visualizer/data/avatar/Box protocol bridge are migrated under Tauri isolation; see `docs/desktop-visualizer-security.md`. Export background tasks remain outside this gate, so the full Tauri product and release stages are not complete.
