...
| ZZZ 抽取价值 | 可在刷新后自动重建 `pull_value_report.md`，并合并 historical_usage / target_coverage / mechanism_review | `python -m zzz_endgame_exporter pull-value` |
| 无 key GPT 交互评审 | 可在刷新后自动生成 `gpt_pull_reviewer_packet.md`，登录后交给 Codex/GPT 做 X+X 评判 | `python -m zzz_endgame_exporter review-packet` |
...
| 机制断点资料 | `configs/zzz_mechanism_notes/*.yaml` 已可接入报告，但来源摘要仍需要随新角色资料更新 | 后续卡池更新时补充技能、专武、影画和攻略来源 |
| HSR 目标账号 pull value | 当前 HSR 已有数据导出与方法论产物，但 pull-value CLI 先落在 ZZZ | 复用 `miho_core.evidence` 做 HSR 角色计划报告适配 |
| 无人值守 GPT 模型评判 | 当前选择不使用 API key，因此不能做到无人值守调用 GPT | 使用 `gpt_pull_reviewer_packet.md` 作为交互版；若未来要全自动，再接入 `OPENAI_API_KEY` |
...
- GPT reviewer 层：你登录后让我读取 `out_zzz/gpt_pull_reviewer_packet.md`，我基于证据包输出 X+X 档位建议、理由和反证。
