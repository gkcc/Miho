...
function loadRec(){try{const raw=JSON.parse(localStorage.getItem(REC_KEY)||'{}');rec={...rec,...raw,elements:raw.elements||{},constraints:raw.constraints&&typeof raw.constraints==='object'&&!Array.isArray(raw.constraints)?raw.constraints:{}};}catch{rec={...rec,elements:{},constraints:{}};}}
...
function constraintSets(mode=rec.mode,scope=rec.scope){const raw=rec.constraints?.[key(mode,scope)]||{},required=new Set((Array.isArray(raw.required)?raw.required:[]).filter(Boolean).slice(0,3)),excluded=new Set((Array.isArray(raw.excluded)?raw.excluded:[]).filter(Boolean).slice(0,160));required.forEach(slug=>excluded.delete(slug));return{required,excluded};}
function setConstraintSets(sets,mode=rec.mode,scope=rec.scope){const order=(a,b)=>(num(charInfo(a).release_order)||99999)-(num(charInfo(b).release_order)||99999)||charName(a).localeCompare(charName(b));rec.constraints[key(mode,scope)]={required:[...sets.required].sort(order),excluded:[...sets.excluded].sort(order)};}
