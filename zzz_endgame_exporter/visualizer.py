...
STYLES_CSS += r""".build{grid-template-columns:46px minmax(150px,.8fr) repeat(6,minmax(78px,1fr)) auto auto auto}@media(max-width:1100px){.build{grid-template-columns:1fr 1fr}}
.rec-constraints{display:grid;grid-template-columns:minmax(330px,.82fr) minmax(0,1.18fr);gap:14px;align-items:center;background:#fff;border:1px solid #d8e1e5;border-radius:8px;padding:12px 14px;margin-bottom:14px}.constraint-picker>label{display:block;color:#2d434c;font-size:13px;font-weight:750;margin-bottom:7px}.constraint-picker>label span{color:#708188;font-size:11px;font-weight:500;margin-left:5px}.constraint-picker-row{display:grid;grid-template-columns:minmax(160px,1fr) auto auto auto;gap:7px}.constraint-picker-row select{min-width:0;height:36px;border:1px solid #c8d4d9;background:#fff;border-radius:6px;padding:6px 8px;color:#172126}.constraint-picker-row button{border:1px solid #2f7767;background:#edf8f3;color:#1d5d4e;border-radius:6px;padding:7px 10px;cursor:pointer;white-space:nowrap;font-weight:700}.constraint-picker-row #recExcludeBtn{border-color:#b77b59;background:#fff4ed;color:#7a3d20}.constraint-picker-row button.quiet{border-color:#c8d4d9;background:#f8fafb;color:#50646c;font-weight:600}.constraint-picker-row button:hover{filter:brightness(.98);box-shadow:0 0 0 1px rgba(23,76,90,.08)}#recConstraintMessage{min-height:16px;margin:5px 0 0;color:#7a5200;font-size:11px}.constraint-groups{display:grid;grid-template-columns:1fr 1fr;gap:10px}.constraint-groups>div{min-width:0;border-left:3px solid #2f7b69;background:#f5faf8;border-radius:5px;padding:8px 9px}.constraint-groups>div+div{border-left-color:#b86b43;background:#fff8f4}.constraint-groups strong{display:block;color:#435860;font-size:11px;margin-bottom:6px}.constraint-chips{display:flex;gap:6px;flex-wrap:wrap;min-height:27px;align-items:center}.constraint-chips:empty::after{content:'未指定';color:#87959a;font-size:11px}.constraint-chip{border:1px solid #79a99d;background:#fff;color:#215d4e;border-radius:999px;padding:4px 8px;cursor:pointer;font-size:11px;font-weight:700}.constraint-chips.excluded .constraint-chip{border-color:#d6a286;color:#7a3d20}.constraint-chip::after{content:' \00d7';color:#6b7c84;margin-left:3px}@media(max-width:900px){.rec-constraints{grid-template-columns:1fr}}@media(max-width:720px){.constraint-picker-row{grid-template-columns:1fr 1fr}.constraint-picker-row select{grid-column:1/-1}.constraint-groups{grid-template-columns:1fr}}
.rec-target-controls{grid-template-columns:minmax(320px,.8fr) minmax(320px,1.2fr)}.rec-target-controls p{align-self:center;margin:0;color:#50656e;font-size:12px;line-height:1.5}@media(max-width:720px){.rec-target-controls{grid-template-columns:1fr}}
"""
...
let DATA=null,state={page:PAGES.has(location.hash.slice(1))?location.hash.slice(1):'box',mode:'sd',role:'all',view:'trend',limit:'16',search:''},box={owned:new Set(),builds:{},buildSlug:'',element:'all',style:'all',status:'all',search:'',saveStatus:'浏览器缓存',exportStatus:''},rec={mode:'sd',scope:'',targetScopes:{},elements:{},constraints:{},gap:'1',riskMode:'warn',limit:'8',search:''},banner={phase:'current',search:''},boxSaveTimer=null,boxSaveRevision=0,boxSaveChain=Promise.resolve(),recConstraintMessage='';
...
function loadRec(){try{const raw=JSON.parse(localStorage.getItem(REC_KEY)||'{}');rec={...rec,...raw,targetScopes:normalizeTargetScopeStore(raw.targetScopes),elements:raw.elements||{},constraints:normalizeConstraintStore(raw.constraints)};}catch{rec={...rec,targetScopes:{},elements:{},constraints:{}};}}
function saveRec(){rec.constraints=normalizeConstraintStore(rec.constraints);localStorage.setItem(REC_KEY,JSON.stringify({...rec,updatedAt:new Date().toISOString()}));}
...
function tierMeta(slug,mode=rec.mode){return DATA.tierRows.filter(r=>r.character_slug===slug&&r.tier_mode===mode).sort((a,b)=>(TIER_RANK[a.tier]??9)-(TIER_RANK[b.tier]??9))[0]||{}}
function memberRisk(m,mode=rec.mode){const risks=[],tier=tierMeta(m.slug,mode),rank=TIER_RANK[tier.tier]??9,bs=m.build;if(m.owned){if(!bs.coreRecorded)risks.push({text:'练度未录入',penalty:m.core?42:22});else if(bs.baseScore<.68)risks.push({text:`练度待补 ${bs.basePercent}%`,penalty:m.core?70:36,severe:m.core});}if(rank>=5)risks.push({text:`${tier.tier}不建议投入${bs.ready?'（已练，降权）':''}`,penalty:bs.ready?35:90,severe:true});else if(rank>=3)risks.push({text:`${tier.tier}非主流低档${bs.ready?'（已练，降权）':''}`,penalty:bs.ready?25:62,severe:true});else if(rank>=1&&!bs.ready)risks.push({text:`${tier.tier}投入谨慎`,penalty:m.core?32:18});return risks;}
function scoreTeam(t,used=new Set()){const selected=elementSet(t.mode,t.scope_key);const members=t.chars.map(slug=>{const info=charInfo(slug),bs=buildState(slug),core=['crit_dps','anomaly_dps'].includes(info.role_group);return{slug,info,build:bs,owned:box.owned.has(slug),selected:selected.has(info.element_cn),core,conflict:used.has(slug)};});members.forEach(m=>m.risks=memberRisk(m,t.mode));const owned=members.filter(m=>m.owned).length,ready=members.filter(m=>m.owned&&m.build.ready).length,miss=3-owned,elementHits=members.filter(m=>m.selected).length,coreHits=members.filter(m=>m.core&&m.selected).length,conflictCount=members.filter(m=>m.conflict&&m.owned).length,risks=members.flatMap(m=>m.risks.map(r=>({...r,name:charName(m.slug)})));if(selected.size&&members.some(m=>m.core)&&coreHits===0)risks.push({text:'主C均未命中推荐属性',penalty:145,severe:true});const penalty=rec.riskMode==='off'?0:risks.reduce((s,r)=>s+(r.penalty||0),0);let score=owned*46+members.filter(m=>m.owned).reduce((s,m)=>s+m.build.score*88,0)-miss*72-conflictCount*160+elementHits*12+coreHits*56+Math.min(num(t.app_rate)||0,35)*2.1-penalty;if(t.rank!=null)score+=Math.max(0,130-t.rank)*.4;if(selected.size&&elementHits===0)score-=35;return{template:t,members,ownedCount:owned,readyCount:ready,missingCount:miss,elementHits,coreHits,conflictCount,risks,score,search:[t.phase_name,t.scope_label,t.bangboo,t.bangboo_name,...t.chars,...t.names_cn,...risks.map(r=>r.text)].join(' ').toLowerCase()};}
...
function rankedFor(mode=rec.mode,scope=rec.scope,used=new Set(),options={}){if(typeof options==='boolean')options={ignoreSearch:options};const constraints=constraintSets(mode,scope),reserved=new Set(options.reserved||[]),maxGap=Number(options.maxGap??rec.gap),riskMode=options.riskMode||rec.riskMode,q=options.ignoreSearch?'':rec.search;return DATA.teamTemplates.filter(t=>t.mode===mode&&t.scope_key===scope&&templateMatchesConstraints(t,constraints)&&!(reserved.size&&(t.chars||[]).some(slug=>reserved.has(slug)))).map(t=>scoreTeam(t,used)).filter(i=>(!Number.isFinite(maxGap)||i.missingCount<=maxGap)&&(riskMode!=='filter'||!i.risks.length)&&(!q||i.search.includes(q))).sort((a,b)=>b.score-a.score||a.conflictCount-b.conflictCount||a.missingCount-b.missingCount||(a.template.rank||9999)-(b.template.rank||9999));}
