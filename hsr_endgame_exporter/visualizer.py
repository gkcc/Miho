...
def write_visualizer_app(
    ...
    *,
    ...
    (visualizer_dir / "styles.css").write_text(_STYLES_CSS + _BUILD_CSS + _RECOMMENDER_CSS, encoding="utf-8")
  ...
  <main class="app-shell">
    ...
    <section id="recommenderView" class="hidden">
      <section class="rec-controls" aria-label="组队推荐控制">
        ...
        <div class="control-group compact"><label>风险</label><select id="recRiskSelect" title="T1及以下、近期走弱或核心属性不匹配时的处理方式"><option value="warn" selected>仅提醒</option><option value="filter">过滤风险</option><option value="off">忽略风险</option></select></div>
...
let box={owned:new Set(),builds:{},buildSlug:'',element:'all',path:'all',role:'all',rarity:'all',status:'all',search:''};
...
fetch('./data.json')
  .then(r=>r.json())
  .then(data=>{DATA=data;loadBox();loadRecSettings();init();render();})
  .catch(err=>{document.body.innerHTML=`<main class="app-shell"><h1>数据加载失败</h1><p>${esc(err.message)}</p></main>`;});

function init(){
  $('metaLine').textContent=`Prydwen T榜更新：${DATA.meta.tierUpdatedAt||DATA.meta.tierUpdatedDate||'未知'} · 本地数据生成：${DATA.meta.generatedAt||'未知'} · Box 本地保存`;
  makeButtons('appTabs',[['analysis','趋势分析'],['box','我的Box'],['recommender','组队推荐']],state.page,v=>{state.page=v;render();});
...
function render(){
  ...
  if(state.page==='box')renderBox();else if(state.page==='recommender')renderRecommender();else renderAnalysis();
}
...
function normalizeBuild(raw={}){const level=BUILD_LEVELS.includes(Number(raw.level))?Number(raw.level):0;const lc=BUILD_LEVELS.includes(Number(raw.lc??raw.lightConeLevel))?Number(raw.lc??raw.lightConeLevel):0;const traceValues=new Set(BUILD_TRACES.map(x=>x[0]));const relicValues=new Set(BUILD_RELICS.map(x=>x[0]));return{level,lc,traces:traceValues.has(raw.traces)?raw.traces:'unset',relics:relicValues.has(raw.relics)?raw.relics:'unset'};}
function buildOptionScore(options,value){return options.find(x=>x[0]===value)?.[2]??0}
function buildRecorded(build){return Boolean(build.level||build.lc||build.traces!=='unset'||build.relics!=='unset')}
function buildState(build){const b=normalizeBuild(build);const traceScore=buildOptionScore(BUILD_TRACES,b.traces);const relicScore=buildOptionScore(BUILD_RELICS,b.relics);const score=(b.level/80)*.25+(b.lc/80)*.2+traceScore*.25+relicScore*.3;const recorded=buildRecorded(b);const ready=recorded&&score>=.86&&b.level>=75&&b.lc>=70&&traceScore>=.82&&relicScore>=.82;let label='练度未录入';if(ready)label='已成型';else if(recorded&&score>=.72)label='可用';else if(recorded)label='待练';return{...b,score,percent:Math.round(score*100),recorded,ready,label};}
function buildFor(slug){return normalizeBuild(box.builds?.[canonicalSlug(slug)]||{})}
function buildShortLabel(slug){const s=buildState(buildFor(slug));return `${s.label}${s.recorded?` ${s.percent}%`:''}`}
function loadBox(){try{const raw=JSON.parse(localStorage.getItem(BOX_KEY)||'{}');const aliases=boxAliasMap();const owned=Array.isArray(raw.owned)?raw.owned:Object.keys(raw.owned||{}).filter(k=>raw.owned[k]);box.owned=new Set(owned.map(s=>aliases.get(s)||s).filter(Boolean));box.builds={};Object.entries(raw.builds||{}).forEach(([slug,build])=>{const resolved=aliases.get(slug)||slug;if(resolved)box.builds[resolved]=normalizeBuild(build);});box.buildSlug=aliases.get(raw.buildSlug)||raw.buildSlug||'';if(box.buildSlug&&!box.owned.has(box.buildSlug))box.buildSlug='';saveBox();}catch{box.owned=new Set();box.builds={};box.buildSlug='';}}
function saveBox(){const builds={};Object.entries(box.builds||{}).forEach(([slug,build])=>{const normalized=normalizeBuild(build);if(buildRecorded(normalized))builds[slug]=normalized;});localStorage.setItem(BOX_KEY,JSON.stringify({version:2,updatedAt:new Date().toISOString(),owned:[...box.owned].sort(),buildSlug:box.buildSlug||'',builds}));}
...
function fullBuild(){return{level:80,lc:80,traces:'max',relics:'great'}}
function setBuildPreset(kind){if(!box.buildSlug)return;if(kind==='clear')delete box.builds[box.buildSlug];else box.builds[box.buildSlug]=fullBuild();box.owned.add(box.buildSlug);saveBox();renderBox();}
function setVisibleBuild(kind){filteredRoster().forEach(r=>{if(kind==='clear')delete box.builds[r.character_slug];else{box.owned.add(r.character_slug);box.builds[r.character_slug]=fullBuild();}});saveBox();renderBox();}
function showBoxTooltip(evt,row){const tt=$('boxTooltip');const owned=box.owned.has(row.character_slug);const state=buildState(buildFor(row.character_slug));tt.hidden=false;tt.innerHTML=`<div class="tooltip-head"><img src="${esc(row.icon_url)}" alt=""><div><strong>${esc(row.character_name_cn||row.character_name_en)}</strong><span>${esc(row.character_name_en)} · ${esc(row.character_slug)}</span></div></div><div class="tooltip-grid"><b>收集状态</b><div>${owned?'已拥有':'未拥有'}</div><b>练度</b><div>${owned?`${esc(state.label)} · ${state.percent}%`:'未拥有'}</div><b>属性/命途</b><div>${esc(row.element_cn||'未知')} · ${esc(row.path_cn||'未知')}</div><b>星级</b><div>${esc(row.rarity||'未知')}</div><b>职能</b><div>${esc(row.role_group_cns||'未分类')}</div><b>排序</b><div>新旧序 #${esc(row.release_order)}</div><b>来源</b><div>${esc(row.source||'')}</div></div>`;moveTooltip(evt);}
...
function exportBox(){const payload={version:2,exportedAt:new Date().toISOString(),owned:[...box.owned].sort(),builds:{}};Object.entries(box.builds||{}).forEach(([slug,build])=>{const normalized=normalizeBuild(build);if(buildRecorded(normalized))payload.builds[slug]=normalized;});const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='hsr_box_state.json';a.click();URL.revokeObjectURL(a.href);}
function importBox(evt){const file=evt.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{try{const data=JSON.parse(String(reader.result||'{}'));const aliases=boxAliasMap();const owned=Array.isArray(data.owned)?data.owned:Object.keys(data.owned||{}).filter(k=>data.owned[k]);box.owned=new Set(owned.map(s=>aliases.get(s)||s).filter(Boolean));box.builds={};Object.entries(data.builds||{}).forEach(([slug,build])=>{const resolved=aliases.get(slug)||slug;if(resolved)box.builds[resolved]=normalizeBuild(build);});box.buildSlug='';saveBox();renderBox();}catch(err){alert(`导入失败：${err.message}`);}finally{evt.target.value='';}};reader.readAsText(file);}
...
function memberRisk(member,mode){
  ...
  if(member.owned){
    if(!build.recorded)reasons.push({type:'build-missing',text:'练度未录入',penalty:core?44:24});
    else if(build.score<.68)reasons.push({type:'build-low',text:`练度待补 ${build.percent}%`,penalty:core?70:38,severe:core});
    else if(build.score<.86)reasons.push({type:'build-mid',text:`练度未成型 ${build.percent}%`,penalty:core?32:16});
  }
  if(tier){
...
function renderRecommender(){
  ...
  const tierRiskLabel=rec.riskMode==='off'?'T档不提醒':'T1及以下提醒';
...
function recMemberHtml(member,item){
  ...
  const buildText=member.owned?` · ${member.buildState.label}`:'';
  return `<div class="rec-member ${member.owned?'owned':'missing'} ${(riskText||coreRisk)&&rec.riskMode!=='off'?'risky':''}" title="${esc([member.owned?'已拥有':'未拥有',member.owned?`练度 ${member.buildState.label} ${member.buildState.percent}%`:'',riskText,coreRisk?'核心属性未命中':''].filter(Boolean).join('；'))}"><img src="${esc(r.icon_url)}" alt="" loading="lazy" decoding="async"><div class="name">${esc(r.character_name_cn||r.character_name_en||member.slug)}</div><div class="meta">${esc(r.element_cn||'')} · ${esc(roleCn(r))}${esc(buildText)}</div></div>`;
}
