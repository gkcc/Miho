...
- **构建边界**：最终候选曾暴露一次“新 desktop 主界面 + 旧内嵌 Visualizer”混合产物；原因是旧 hotfix checkout 共用 release target，旧 `miho-core` rlib 时间戳晚于当前静态资源，Cargo 误判 fresh。清理 release `miho-core` 后完整重建解决，最终三份产物均直接检出 `miho-visualizer-ready-v1`。以后 detached/hotfix checkout 必须使用独立 target-dir，正式交付还要对最终 desktop EXE 做内嵌 marker 哨兵，不能只核对 AppData 生成物。
