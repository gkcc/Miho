...
5. 默认交付是直接构建并更新当前 Tauri desktop；若改动共享数据生成链，必须同时更新同版本 CLI 与每日任务 generation。NSIS、安装器、portable 和发布包只在用户明确要求时进入范围。
...
- 当前桌面与数据 CLI 直接由 Tauri/Rust Release 构建；桌面更新到 `D:\Miho Endgame\miho-desktop.exe`，共享数据生成链变化时同步 `D:\Miho Endgame\miho.exe` 并通过 owner-aware 事务切换每日任务 generation。Box 与 AppData 保持原位；当前二进制以直接构建哈希、exact health 和后台产品探针为交付证据。
...
- 运行时变化默认重建并直接更新 Tauri desktop；共享数据生成链变化同时更新 CLI 和每日任务 generation。NSIS/portable/发布包仅在用户明确要求时重建，纯文档提交不触发程序重建。
...
| 2026-07-26 | 涉及 `miho-core`、`miho-app`、`miho-cli` 或 Visualizer 数据契约的改动，直接替换 desktop 不再构成完整交付 | 默认构建同时生成 desktop/CLI；先以 candidate run + exact health 验证，再原子切换 owner-aware 每日任务 generation，避免旧 CLI 把新数据覆盖回 legacy 格式 | 数据生成边界、update config、generation ownership 或 health 契约变化时 |
...
| desktop 与每日任务 CLI generation 漂移，旧 CLI 成功退出却把新数据覆盖回 legacy 格式 | 低（2026-07-26 已修复并真机重放） | 默认构建同步 desktop/CLI；共享数据链变化必须经过 candidate run、exact health、owner-aware generation 切换及正式任务 Running→Ready 复核 |
| PowerShell launcher 丢失 native `$LASTEXITCODE` 或业务失败后前移 state | 低（runner/launcher 与当前 owner-aware 真实任务均已验证） | PowerShell 仅调用 `update run/health`；WinPS 5.1 与 pwsh 7 EAP/退出码 0/2/7 回归；成功 state 只由 Rust 事务提交 |
...
### ZZZ 卫星展示与诺姆身份归一（2026-07-17，已交付）

- **根因与产品修复**：官方 roster/Prydwen 已使用 `norma`，卡池计划、决策基线和机制笔记仍使用旧 `nom`；Visualizer 只按精确 slug 合并，因此生成两张“诺姆·霍洛维尔”。生产配置现统一为 `norma`。卡池“全部”仍按 `current → next → satellite → previous` 展示运营阶段；Box 曾错误复用该阶段优先级，现已按 `release_order` 恢复实际实装顺序，卡池状态只作标签和筛选。
- **旧状态兼容**：前端加载时归一 stale `nom → norma`，合并 roster/banner 同身份行，并迁移 Box owned、builds、buildSlug 及按关卡保存的必上/排除约束；若两份练度同时存在则逐字段保留更完整进度。Workspace bootstrap 新种子为 `norma.yaml`，旧受管 `nom.yaml` 从 ownership 退役但不删除或改写用户文件。Rust/Python 两套 ZZZ `app.js` 字节一致，当前静态 SHA-256 为 `4AB29F1F579657AC757C5DD5521AF5A70358838BDAA77F3F7BC3CDC752EF9809`。
- **生产数据与真实入口证据**：本机受管配置升级后重新生成 ZZZ visualizer，原始 roster 从 59 降为 58；诺姆唯一为 `norma/current`，卫星 `remiel/sigrid` 均保留。最终安装版隐藏窗口 CDP/DOM 实跑 ZZZ → HSR → ZZZ：Box 首 10 张精确为 `norma / velina / pyrois / billy-starlight / promeia / cissia / nangong-yu / aria / sunna / ye-shunguang`，即 `release_order=0..9`；千夏虽为当期复刻仍在第 9，卫星 Remiel/Sigrid 在第 57/58。卡池页当期 4 人、全部分析模式和图片门禁同时通过。
...
- **仓库与安装版数据证据**：首次联网重建仓库 `out_zzz` 只证明开发输出，不代表安装版已经更新。随后已在安装版真实工作区 `C:\Users\zy958\AppData\Roaming\com.miho.endgame` 事务式重建 ZZZ；`name_map` 为 `norma=0 / velina=1 / pyrois=2`，Pyrois 显示“佩洛伊斯”、`kind=agent`、无需人工确认，并已从 unresolved 移除；usage、当前 Tier 与 roster 同步回填中文。最终真实安装版 `#boxGrid` 共 58 张卡，Pyrois 唯一且位于索引 2，即第 3 张。
...
### 每日更新 generation 同步与刷新后页签保持（2026-07-26，已交付）

- **现场根因与修复**：7 月 26 日 09:30 的每日任务虽然返回成功，实际仍绑定 7 月 18 日旧 CLI，随后把新版输出覆盖成只有 `data.json` 的 legacy 数据，缺少 `data.v2.json`、`freshness`、`data_quality` 和 `bannerRefresh`。旧格式现在会明确提示兼容降级；freshness 按采样日、期次、snapshot 与主题精确匹配，旧期队伍不会再借新期状态标成“当前”；更新完成后桌面保留用户所在的卡池/终局 TAB。
- **构建与自动化闭环**：`pnpm run tauri:build` 现同时构建 desktop 与 CLI。直接交付的 `D:\Miho Endgame\miho-desktop.exe` SHA-256 为 `FC1F1A63AF87829A29E38379FE4236F74F349590B766C6B05004675655D20907`，`D:\Miho Endgame\miho.exe` 为 `A9B38862D9345A69C96DD73986A8339411CF3513A2836E49B6613B05FA8715A9`；任务 `MihoEndgameDailyUpdate-4bfbc997e809d2ec` 已切到 generation `miho-0.1.0-a9b38862d9345a69c96dd73986a8339411cf3513a2836e49b6613b05fa8715a9`。正式重放明确经过 Running → Ready，`LastTaskResult=0`，attempt `20260726T085143248116Z-80508-0` 对 HSR/ZZZ exact health 均为 `healthy=true`。
- **当前数据结论**：ZZZ 的 `2026-07-19` 是终局统计采样，`2026-07-08` 是 Prydwen 榜单更新时间；式舆防卫 `3.0.2`（`2026-07-10` 至 `2026-07-24`）已结束，危局强袭战 `3.0.3`（`2026-07-17` 至 `2026-07-29`）为当前期。HSR 最新采样仍为 `2026-06-25`，原因是上游 `LvlUrArti/MocDataProcessed` 最新配置仍停在 `4.3.2 / collect_date 2026-06-25`；PF“借虚成真”仍在 `2026-06-22` 至 `2026-08-03` 周期，MoC“值日行动”、AS“遗忘冽风”、AA“嗡鸣如笑”已过期，产品保留真实历史状态而不伪造新鲜度。两游戏官方卡池刷新均为 fresh，最终 fetched_at 为 `2026-07-26 16:51:43 +08:00`。
...
### 启动黑框、每日任务无窗化与刷新关闭竞态收口（2026-07-26，已交付）

- **黑框根因与修复**：桌面快捷方式直接指向 `D:\Miho Endgame\miho-desktop.exe`，没有 cmd/PowerShell 包装；实际弹出的 Windows Terminal 来自启动后自动化归属探针所创建的 PowerShell 子进程。该探针现统一使用 Windows `CREATE_NO_WINDOW`。每日 09:30 任务原本又直接执行 console-subsystem CLI，现保留普通 `miho.exe` 的控制台语义，同时用 `automation-no-window` feature 生成独立 WINDOWS_GUI CLI；正式任务 action 已指向该无窗 generation，避免启动和定时更新两条路径打断前台游戏。
- **刷新与关闭一致性**：Box 瞬时 PUT 失败会保留并重发 payload，`flushBoxSave()` 会排空保存期间产生的新 revision；iframe 不再用原生 `load` 冒充数据就绪，而以 `navigation_id + data_revision` 的 ready 握手绑定当前导航，未 ready 页面始终 hidden/inert，超时和晚到回执不能复活旧页。关闭会等待在途任务启动、工作区切换与双 Box flush；关闭期间更新、报告、切换、取消和文件操作均锁定，关闭取消后按后端真实 workspace 对账。
- **构建边界**：最终候选曾暴露一次“新 desktop 主界面 + 旧内嵌 Visualizer”混合产物；原因是旧 hotfix checkout 共用 release target，旧 `miho-core` rlib 时间戳晚于当前静态资源，Cargo 误判 fresh。清理 release `miho-core` 后完整重建解决，最终三份产物均直接检出 `miho-visualizer-ready-v1`。以后 detached/hotfix checkout 必须使用独立 target-dir；后续里程碑已把最终 EXE 的完整静态资源字节校验接入正式构建，不能只核对 AppData 生成物。
- **验证与直接交付**：CLI 定点测试 71 项、前端/刷新/推荐契约 81 项、Python Visualizer 59 passed（2 live deselected）、Rust fmt、diff check、TypeScript/Vite 与完整 `pnpm run tauri:build` 均通过。候选及真实安装路径的后台 CDP/DOM 深探针都覆盖 ZZZ → HSR → ZZZ、Box、六模式期次/主题/周期、卡池、页面保持和关闭，均正常退出 0、stdout/stderr 为空、无残留进程；WindowsTerminal/OpenConsole PID 集合前后完全一致。安装版 desktop/普通 CLI SHA-256 分别为 `0190E45D6B280A6A39E1B32777A05101E944C24331A8B6DD51D1BE4D4BCBA159` / `9AA139E7053650BBA2F6A9D36A6617F60C2A14B67290321A97059729A64E1180`；每日任务 generation 为 `miho-0.1.0-78262012a24eeff068218bd2cb06ac08e7e90913726c1c4518837d0750cb6b50`，PE Subsystem=2，任务 Ready/Enabled、`LastTaskResult=0`。HSR/ZZZ Box SHA-256 仍分别为 `E0476FAA415CDD651DB69FD4B969E1C91FAB69C87DFEC86DE377183F009AC662` / `C677733006B569CBFB96EB95BF1827FB26A58292C1F17A9180AC7FF01E4D7491`；未进入 NSIS、portable 或旧安装器链。

### 最终产物字节门禁与关闭协调器可执行化（2026-07-26，已交付）

- **最终产物门禁**：`tauri:build` 末尾现直接读取当前 HSR/ZZZ 的 `index.html`、`app.js`、`styles.css` 和共享 `solver.js` 原始字节，并要求它们在 Desktop、普通 CLI、无窗任务 CLI 三份最终 EXE 中各恰好出现一次。缺失、任意字节漂移或重复嵌入都会令构建失败；合成二进制正例、缺失、改单字节和重复反例均已固化为 Node 测试，交付契约同时锁定门禁必须在 Desktop build 之后且为最后一步。
...
### Generation 绑定的 freshness、可访问健康卡与紧凑产品回执（2026-07-27，已交付）

- **跨代一致性**：update runner 现在在 workspace 写租约释放前读取并验证 `data_quality.json`，利用既有 generation receipt 与 state artifact 清单，按 attempt、config digest、文件大小和 SHA-256 绑定并复验 exact committed generation；TaskManager 不再在任务结束后裸读可能已被下一代覆盖的工作区。Desktop health 的 config、state、receipt、artifact 与 freshness 也全部来自同一个 snapshot lease，消除了刷新交叠时把旧 attempt 和新数据拼成 false-green 的窗口。
- **终局状态可见性**：健康卡改为原生 `details/summary`，支持键盘、触屏与读屏展开，逐模式显示状态、采样日和周期边界；终局分析页继续显示期次、主题、起止日期和当前/历史状态，精确 UTC 完成时间由 `data-completed-at-utc` 暴露给探针。联网重放后 ZZZ 终局采样仍为 `2026-07-19`，榜单更新时间仍为 `2026-07-08`；HSR 本机网络更新成功，但上游终局样本仍为 `2026-06-25`，因此产品保持 warning 并明确属于上游历史样本。
...
### Freshness 发布前门禁与逐游戏 generation 日期闭环（2026-07-27，已交付）

- **发布前 last-good 保护**：共享严格 validator 统一检查 schema、游戏、状态、模式全集、日期格式、周期边界及 generation 相对关系；producer/exporter 在输出目录或 Hub swap 前验证，因此 producer 生成的非法 freshness 不会覆盖该游戏的 last-good。runner 在 success state/receipt commit 前再次验证，提交前发现异常不会推进成功 state；错误稳定传播为 `update.health_freshness_invalid`。配置同时拒绝重复 mode；公开 freshness 日期与 core 共用严格 `YYYY-MM-DD` / RFC3339 语义，`unknown` 仍允许上游不可解析日期但公开层会清空。
- **跨代 health 纠正**：联合 health 不再把最新 canonical 日期套给两个游戏，而是按 HSR、ZZZ 各自 state 绑定的 generation receipt 取得生成日并重验对应 artifact。commit 后 artifact 替换复验继续保留，生产 CLI/Tauri 会 fail-closed；这一复验用于检测提交后的外部替换，不声称回滚已经提交的 state/output。当前也不声称 HSR/ZZZ 两个游戏的整批输出跨游戏原子发布。
...
### ZZZ 3.1 双池刷新与无窗任务回归门禁（2026-07-27，已交付）

- **卡池增量修复**：ZZZ 官方裸标题 `3.1版本限时频段` 现会结合更新通知计算 `2026-07-29 11:00` 起点；HTML 内联标签不再切断角色名，官方短名“蕾米埃尔”可唯一解析到既有卫星“蕾米埃尔·丹”，歧义继续 fail-closed。合并器会隔离同一次响应中新追加的阶段，避免两组池因共享派派/赛斯而互相覆盖；仅抓到历史公告时标记 `no_current`，界面明确说明官方响应未覆盖当前或下一期。
...
### HSR/ZZZ 组队推荐能力对齐与三队真实路径闭环（2026-08-15，已交付）

- **差异根因与产品对齐**：`cfd8ee5` 只为 HSR 增加了“末层实战 / 按弱点配队”双路径，ZZZ 仍停留在具体关卡推荐。ZZZ 现完整补齐“末层实战 / 按属性配队”、按模式 2/3 队、当前最新具体关卡全池、三代理人无序去重、逐队弱点与必须/排除约束、策略隔离锁及旧状态迁移、核心代理人属性命中、A/B/C 证据披露、C 级理论替补、最终阵容评分/风险重算和全局不复用；HSR 同步修正替补后重算、`A > duplicate_count > Rank` 去重、约束文案与原模板 Box 披露。ZZZ 自定义策略下的代理人约束提示也会随“当前队伍 / 当前关卡”切换，不再保留错误悬浮文案。
- **三队求解与新增角色头像**：真实 4.4.2 AS 数据会把 68,257 个合格变体截成高度重叠的前 240 个，旧 beam 因而显示 `0/3`，并非无可行三队。双游戏现于截断前保留一组完整不复用种子，共享 solver 优先保留仍可覆盖剩余关卡的中间状态，最终 HSR 三队真实入口可稳定成套。8 月 11 日英文 HoYoWiki 新增 Pearl、中文源尚未同步，导致 HSR 93 个 roster 只有 92 张本地头像；发布内置种子已按既有 160×160 WebP 管线补入官方 Pearl 图标，继续保持外链 fail-closed，正式数据为 `./assets/avatars/pearl.webp`。
...
### ZZZ 新角色公告导致连续更新失败（2026-09-09，已交付）

- **根因与修复**：9 月 7 日发布的 3.2 上期公告包含克拉蕾，HoYoWiki 双语角色目录尚未收录；原卡池合并抛出 missing name_map，导致整次 ZZZ export 失败。HSR/ZZZ 现把身份未绑定的整期官方事实保存在 `refresh.pending_phases`，显示姓名、角色定位、日期、来源和“角色资料待同步”，不臆造 slug 或混入卡池计算；目录补齐后自动恢复。歧义/非法身份、坏公告和网络失败仍拒绝。ZZZ 同时补齐从完整官方目录读取名字，避免角色尚无统计/Tier 就被遗漏；预留日期组归属，防止恢复时共享复刻角色抢占其他卡池，pending 清理兼容 legacy ID。
...
- 当前默认构建/验证：`pnpm run tauri:build` 同时生成 `target/release/miho-desktop.exe` 与 `target/release/miho.exe`，按风险运行相关 Rust/前端定点测试，再用后台 CDP/DOM 探针验证真实入口；共享数据生成链变化时还要以 candidate run + exact health 同步每日任务 generation。全 workspace 回归只用于大范围变化；`scripts/build_rust_app.ps1 -Release` 及其 NSIS/manifest 门禁仅在用户明确要求旧发布链时运行。
