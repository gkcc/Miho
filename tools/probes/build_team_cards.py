...
def owned_member(
    ...
    *,
    ...
    return {
        ...
        "confidence": "high" if snapshot_jsons.get(character) else "medium",
        "reason": reason,
    }
...
def build_cards(planner_report: dict[str, Any], action_report: dict[str, Any]) -> list[dict[str, Any]]:
...
def render_markdown(team_report: dict[str, Any]) -> str:
    lines = [
        "# 高难配队候选卡",
        "",
        "队伍候选基于本地快照和本地 catalog；catalog candidate 不代表已拥有。",
        "",
        "## Summary",
        "",
    ]
...
def build_team_cards(
    *,
    ...
    cards = build_cards(planner, action_report)
    result = {
