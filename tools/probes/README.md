...
```powershell
dist\MihoProbe.exe box-value --roster-json data\probes\box\1782845579231_roster_manual_draft.json --meta-snapshot data\probes\meta\zzz_prydwen_meta_all_phases.json
```
...
`box-status` 只读检查本地文件，不跑 OCR、不联网；默认只扫描 `data/probes/exported_images/`，避免把 `figs/` 中的角色详情分享图误当作 box 总览图。需要扫描其他目录时显式传 `--image-dir`。它会输出 `box_status_freshness`：`source_hash_match` 表示 roster probe 的源图 hash 命中最新 box 图，可复用 roster；`source_hash_mismatch` 表示最新 box 图和 roster 源图不同，应先刷新 roster；旧 roster 缺少 hash 时才退回 `roster_stale_by_mtime` / `current_or_unknown`。它也会输出 `box_status_roster_quality`、`box_status_roster_needs_review_count`、`box_status_review_gate`、`box_status_blocks_accepted_roster`、`box_status_manual_confirmation_required_before_accepted_roster`、`box_status_roster_review_markdown` 和 `box_status_roster_review_markdown_status`；`needs_manual_review` 表示可以继续生成 probe 价值报告，但不能把该 roster 当作 accepted roster，`box_status_roster_review_markdown` 指向应人工核对的 Markdown。若复核 Markdown 缺失，`review_gate=review_markdown_missing` 会阻断 accepted roster；若 Markdown 旧于 roster JSON，`box_status_roster_review_markdown_status=stale` 也只能先重新生成复核材料；若状态为 `unknown`，也必须先重新生成复核材料。此时 `box_status_review_repair_command` 会给出覆盖当前 roster JSON / Markdown 的 `box-roster` 命令；它不会被 `box-status` 自动执行，`box_status_review_repair_command_execution=manual_only` 且 `box_status_review_repair_writes_probe_files=True` 时只写本地 probe 文件。如果 roster 源图已不匹配或 roster 本身需要刷新，repair command 会被阻断，先执行 `box_status_next` 刷新 roster。`--box-image` 只读取用户显式提供的本地图片，输出脱敏 `roster_from_box_image.json` 和复核 Markdown；不保存 UID / 昵称 / header 原始 OCR，不读取 cookie/token，不写正式数据库。若 `needs_review_count > 0`，价值报告仍可生成，但该 roster 只能当作 probe 草案，人工确认前不得进入 accepted roster。

`box-status` 决策表：

| 状态组合 | 可执行动作 |
| --- | --- |
| `box_status_freshness=source_hash_mismatch` 或 `roster_stale_by_mtime` | 执行 `box_status_next`，先刷新 roster。 |
| `review_gate=review_markdown_missing/stale/unknown` 且 `box_status_review_repair_command_status=available` | 手动执行 `box_status_review_repair_command`，只重建当前 roster 的 probe JSON/Markdown 复核材料。 |
| `box_status_review_repair_command_status=blocked_by_roster_refresh` | 不执行 repair command；先执行 `box_status_next`。 |
| `box_status_blocks_accepted_roster=True` | 只能继续做 probe 报告或人工复核，不能汇入 accepted roster。 |
| `box_status_blocks_accepted_roster=False` 且 `box_status_manual_confirmation_required_before_accepted_roster=True` | 没有自动阻断，但仍必须人工确认后才能汇入 accepted roster。 |
| `review_gate=quality_ok_manual_confirmation_required` 且复核 Markdown 为 `current` | 可继续生成 probe 价值报告；人工确认前不得进入 accepted roster。 |
...
* accepted 记录还会写入 `accept_evidence`：说明人工接收、复核预览、normalized JSON hash、unsafe accept 检查和 roster index 命中情况，Dashboard 会直接展示这段证据；
* 如果 receipt 显示 accepted snapshot 已写出但没有进入 `roster_index`，Demo Doctor 会阻断 `try_now`，要求先修复 evidence mismatch；
