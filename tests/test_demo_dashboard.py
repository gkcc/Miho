...
class DemoDashboardTests(unittest.TestCase):
    ...
    def test_dashboard_shows_app_export_readiness_panel(self) -> None:
        ...
        summary["app_export_readiness"] = {
            ...
            "calibration_report_html": "data/probes/demo/app_export_workflow/miyoushe_app_export_calibration_report.html",
            "calibration_report_json": "data/probes/demo/app_export_workflow/miyoushe_app_export_calibration_report.json",
            "calibration_screenshot": "data/probes/demo/app_export_workflow/window_grid.png",
            "calibration_status": "screenshot_captured",
    ...
    def test_action_checklist_surfaces_review_preview_gate_reasons(self) -> None:
        ...
        summary["review_decision_preview"] = {
            ...
            "summary": {
                "accept_count": 1,
                "blocked_accept_count": 1,
                "override_accept_count": 0,
                "would_update_roster_count": 0,
            },
            "items": [
                {
                    "character": "星见雅",
    ...
    def test_action_checklist_surfaces_source_check_blockers(self) -> None:
        summary = dashboard_minimal_summary()
        summary["action_checklist"] = {
            "checklist_status": "blocked",
            "summary": {"item_count": 0, "ready_count": 0, "needs_review_count": 0, "blocked_count": 1},
        ...
        summary["review_decision_preview"] = {
            ...
            "source_check": {
                ...
                "warnings": ["source_review_inbox_sha256 缺失，无法确认模板来源。"],
            },
    ...
    def test_action_checklist_marks_ready_with_override_as_safe_apply_state(self) -> None:
        summary = dashboard_minimal_summary()
        summary["action_checklist"] = {
            "checklist_status": "ready",
            "summary": {"item_count": 1, "ready_count": 1, "needs_review_count": 0, "blocked_count": 0},
            "warnings": [],
            "safe_apply_command": "python tools/probes/apply_review_decisions.py --require-preview-ready",
        }
        summary["review_decision_preview"] = {
            "preview_status": "ready_with_override",
            "summary": {"would_update_roster_count": 1},
            "items": [
                {
                    "character": "星见雅",
                    "decision": "accept",
                    "decision_status": "ready_with_override",
                    "warnings": ["quality blockers 由 note/override_reason 解释，仅作为 dry-run 预览。"],
                }
            ],
        }
    ...
    def test_dashboard_shows_final_brief_before_details(self) -> None:
        ...
        self.assertIn("已执行，后续诊断有警告", html)
        self.assertIn("记录归属", html)
        self.assertIn("匹配当前页面", html)
        self.assertIn("后续诊断匹配当前页面", html)
        self.assertIn("操作记录来源", html)
        self.assertIn("后续诊断匹配当前页面", html)
        self.assertIn("匹配依据", html)
        self.assertIn("后续诊断", html)
        self.assertIn("执行脚本路径", html)
        self.assertIn("重跑开始时间", html)
        self.assertIn("后续诊断状态", html)
        self.assertIn("后续诊断已随重跑更新", html)
        self.assertNotIn("executed_with_followup_warning", html)
        self.assertNotIn("launcher report freshness", html)
        self.assertNotIn("follow_up_matches_current_doctor", html)
        self.assertNotIn("operation_state", html)
        self.assertNotIn("freshness_match_source", html)
        self.assertNotIn("command_script_resolved", html)
        self.assertNotIn("rerun_started_at", html)
        self.assertNotIn("follow_up.doctor_status", html)
        self.assertNotIn("follow_up.updated_after_rerun", html)
    ...
    def test_dashboard_update_command_ready_is_copy_only(self) -> None:
        ...
        self.assertIn("仅打印命令", html)
        self.assertNotIn("<button", html)
        self.assertNotIn(">printed<", html)
    ...
    def test_dashboard_launcher_ready_to_try_is_read_only(self) -> None:
        ...
        self.assertIn("后续下一步", html)
        self.assertIn("页面刷新状态", html)
        self.assertIn("已刷新", html)
        self.assertIn("按执行清单去游戏内尝试", html)
        self.assertIn("launcher_report_ready.json", html)
        self.assertNotIn("follow_up.primary_next_action", html)
        self.assertNotIn("dashboard_refresh", html)
    ...
    def test_initial_current_with_stale_followup_suppresses_try_now(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("启动器记录匹配启动前诊断", html)
            self.assertIn("后续诊断仅供审计", html)
    ...
    def test_launcher_report_summary_marks_stale_and_dashboard_suppresses_action_state(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("历史启动器记录，仅供审计", html)
    ...
    def test_dashboard_html_contains_case_links_and_quality(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("保值命中", html)
            self.assertNotIn("Tier / 保值观察", html)
            self.assertNotIn("Tier 命中", html)
            self.assertIn("运行一致性", html)
    ...
    def test_run_demo_pipeline_includes_app_export_readiness_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            calibration_report = workflow_dir / "miyoushe_app_export_calibration_report.json"
            calibration_report_html = workflow_dir / "miyoushe_app_export_calibration_report.html"
            calibration_screenshot = workflow_dir / "window_grid.png"
            ...
            calibration_screenshot.write_bytes(b"fake")
            calibration_report.write_text(
                json.dumps(
                    {
                        "schema_version": "p4.5-miyoushe-app-export-calibration-report",
                        "status": "screenshot_captured",
                        "next_action": "照着网格截图，把每个步骤的 x/y 填入校准清单。",
                        "screenshot": {"path": str(calibration_screenshot)},
                    },
                    ensure_ascii=False,
                ),
                encoding="utf-8",
            )
            calibration_report_html.write_text("<!doctype html><title>calibration</title>", encoding="utf-8")
            manifest_path = root / "empty_manifest.json"
            manifest_path.write_text(json.dumps({"cases": []}, ensure_ascii=False), encoding="utf-8")
            ...
            self.assertEqual(summary["app_export_readiness"]["calibration_report_json"], str(calibration_report))
            self.assertEqual(summary["app_export_readiness"]["calibration_report_html"], str(calibration_report_html))
            self.assertEqual(summary["app_export_readiness"]["calibration_screenshot"], str(calibration_screenshot))
            dashboard_html = Path(summary["dashboard_html"]).read_text(encoding="utf-8")
            self.assertIn("一键更新练度准备状态", dashboard_html)
            self.assertIn("已沉淀路线，等待校准", dashboard_html)
            self.assertIn("先生成网格截图", dashboard_html)
            self.assertIn("校准清单状态", dashboard_html)
            self.assertIn("APP 导出校准清单", dashboard_html)
            self.assertIn("APP 导出校准报告", dashboard_html)
    ...
    def test_run_demo_pipeline_with_targets_generates_training_plan(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotIn("Tier / 保值观察", dashboard_html)
