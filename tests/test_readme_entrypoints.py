...
class ReadmeEntrypointTests(unittest.TestCase):
    ...
    def test_readme_stays_product_facing(self) -> None:
        ...
        self.assertIn("Opening cached Dashboard only. OCR will NOT run.", readme)
        ...
        self.assertIn("不会自动掉进慢 OCR 脚本", readme)
    ...
    def test_demo_launcher_default_never_runs_ocr(self) -> None:
        ...
        self.assertIn("Opening cached Dashboard only. OCR will NOT run.", wrapper)
        self.assertIn("Fresh OCR requested. This can be slow", wrapper)
        self.assertIn("For UI acceptance", wrapper)
        self.assertIn("build_miho_probe_exe.bat", wrapper)
        self.assertIn("EXE-first", wrapper)
        self.assertIn("will not fall back to the slow Python/OCR path by default", wrapper)
